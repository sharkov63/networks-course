...
3. Сколько байт в IP-заголовке? Сколько байт приходится на полезную нагрузку IP-дейтаграммы?
   ...
   - ![](./images/Wireshark_3a.png)
   - Полезная нагрузка IP-дейтаграммы: 36 байт, где 8 байт из UDP-заголовка, и 28 байт полезных данных в UDP.
4. Отсортируйте отслеженные пакеты по их исходному IP-адресу; для этого щелкните по
   ...
   1. Какие поля IP-дейтаграммы всегда изменяются от одной дейтаграммы к
      ...
      - ![](./images/Wireshark_4.1.png)
      - Каждый раз меняется поле IP-идентификатора, и контрольной суммы (это следствие первого).
   2. Какие поля не меняются? Какие поля должны оставаться неизменными? Какие
      ...
      - Не меняется поле версии IPv4, длины заголовка в 20 байт, типа обслуживания, длины дейтаграммы в 56 байт, поле флагов и смещение с нулевыми значениями, поле протокола верхнего уровня, а также поля исходного и конечного IP-адресов.
      - Должны оставаться неизменными поле версии IPv4 (оно определяется заданным пользователем адресом назначения), длин заголовка и всей дейтаграммы (пользователь задал длину дейтаграммы в 56 байт), поле протокола UDP, поля исходного и конечного адресов (мы должны отправлять туда же пакеты).
      - Должно меняется поле-идентификатор (ведь это разные пакеты), контрольная сумма, TTL (но раз в три пакета).
...
6. Остаются ли эти значения (Identification и TTL) неизменными во всех сообщениях
   ...
   - Идентификатор увеличивается на 1, а TTL остается неизменным 64.
...
8. **Фрагментация**. Остановите захват пакетов в Wireshark. Измените размер пакета
   ...
   1. Было ли это сообщение фрагментировано между двумя или более IP-дейтаграммами? 
      ...
      - ![](./images/Wireshark_8i.png)
      - UDP-сегмент был разбит на три фрагмента.
   2. Какие поля IP-заголовка изменяются в разных фрагментах?
      - ![](./images/Wireshark_8ii.png)
      - Изменяется fragment offset, контрольная сумма. Последний фрагмент имеет другую длину (потому что разбивается по 1500 байт), и другие флаги (обозначающие, что это последний фрагмент и больше нет).
