...
class MyMultithreadedServer:
    ...
    def launch(self):
        ...
        try:
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=self._max_workers) as thread_pool:
                while True:
                    connection, addr = listen_socket.accept()
                    print("New connection on addr", addr)
                    thread_pool.submit(self._serve_client, connection)
        finally:
...
def main():
    if len(sys.argv) != 3:
        print("Invalid number of arguments {}; expected two arguments <port> and <max_workers>".format(
            len(sys.argv) - 1))
        exit(1)
    raw_port = sys.argv[1]
    raw_max_workers = sys.argv[2]
    try:
        port = int(raw_port)
        ...
        exit(1)
    server = MyMultithreadedServer(port, max_workers)
    server.launch()
