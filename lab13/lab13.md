...
1. Поверх какого протокола посылаются сообщения DHCP – UDP или TCP?
   - Поверх UDP
   - ![](./images/Wireshark_DHCP_1.png)
2. Каков адрес канального уровня (т.е., Ethernet-адрес) у вашего хоста?
   - `e4:aa:ea:f6:e5:d7`
   - ![](./images/Wireshark_DHCP_2.png)
3. Каковы значения Transaction-ID в наборе (Request/ACK) DHCP-сообщений? Опишите
   назначение поля Transaction-ID.
   - ![](./images/Wireshark_DHCP_3.png)
   - Transaction ID - случайное число, выбранное клиентом, которое идентифицирует одну пересылку между клиентом и сервером (транзакцию).
4. Хост использует DHCP-протокол, в частности, для получения IP-адреса. Но IP-адрес хоста
   ...
   - Используется IP-адрес 255.255.255.255 для широковещательной рассылки
   - ![](./images/Wireshark_DHCP_4.png)
   - Discover: `0.0.0.0`, `255.255.255.255`
   - Offer: `192.168.1.1`, `192.168.1.120`
   - Request: `0.0.0.0`, `255.255.255.255`
   - ACK: `192.168.1.1`, `192.168.1.120`
5. Каков IP-адрес вашего DHCP-сервера?
   - `192.168.1.1`
6. Объясните назначение срока аренды IP-адреса. Какова длительность срока аренды в
   вашем эксперименте?
   - ![](./images/Wireshark_DHCP_6.png)
   - IP-адреса - это ограниченный ресурс, и его нужно распределять между потенциально неограниченным числом хостов. Поэтому, нельзя выдавать IP-адрес навсегда (хост может потом уже не использоваться), значит надо выдавать на сколько-то. Срок аренды говорит, на сколько.
   - У меня 7 часов
...
#### Решение
1. Продифференцируем и получим оптимальную вероятность $p=1/N$: ![](./images/Problem1_1.png)
2. Предел эффективности равен $1/e$: ![](./images/Problem1_2.png)
...
#### Решение
1. В предыдущих 4 квантах узел A должен не пытаться передавать, а остальные могут делать, что хотят. В кванте 5 же узел А должен передавать, а остальные --- не передавать. Значит, вероятность равна $(1-p)^4 \cdot p(1-p)^3=p(1-p)^7$.
2. Эти три события несовместимы (только один узел может передать в кванте 4). Аналогично пункту 1, вероятность того, что конкретный узел с первого раза передаст в кванте 4, равна $(1-p)^3\cdot p (1-p)^3=p(1-p)^6$. Таким образом, ответ равен $3p(1-p)^6$.
3. Вероятность успешной передачи в конкретный квант равна $4p(1-p)^3$ (один узел передает, остальные не передают). Первая успешная передача в кванте 3, тогда и только тогда, когда в первых двух нет передачи или коллизия, а в третьей успешная передача. Значит, ответ равен $(1-4p(1-p)^3)^2 \cdot 4p(1-p)^3$.
4. Эффективность (доля времени, когда происходит полезная передача) равна $4p(1-p)^3$ --- вероятности, что в конкретный квант будет передача.
