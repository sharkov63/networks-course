...
#### Демонстрация работы
* `python3 src/client.py` - клиент, см хелпу.
* `python3 src/server.py` - клиент, см хелпу.
* Пример работы. Здесь сразу же сделаны задания Б и В: ![](./images/snw.png). Сначала клиент передает серверу файл `./test.txt`, затем сервер передает клиенту файл `./test2.txt`
...
`./src/checksum.py` - реализация; `./src/checksum_test.py` - тесты. Запуск тестов: `python3 ./src/checksum_test.py`
