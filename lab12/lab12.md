...
```
число маршрутизаторов n (они пронумерованы от 0 до n-1)
IP маршрутизатора 0
IP маршрутизатора 1
IP маршрутизатора 2
...
IP маршрутизатора n-1
список связанных с маршрутизатором 0
список связанных с маршрутизатором 1
...
список связанных с маршрутизатором n-1
```

Пример в файле `example.txt`

Граф должен быть неориентированный и связный.

#### Запуск

```shell
python3 src/rip.py < example.txt
```
