...
1. Каков 48-разрядный Ethernet-адрес вашего компьютера?
   - `ifconfig`: ![](./media/Wireshark_1a.png)
2. Какому устройству соответствует значение Ethernet-адреса?
   ...
   - Этот MAC-адрес соответствует Wi-Fi адаптеру Qualcomm Atheros "QCA6174 802.11ac Wireless Network Adapter". Хотя тут тоже ещё компания Lite-On: MAC-адрес принадлежит ей и в WireShark тоже написано `Liteon`. Я так и не понял кто здесь кто.
   - ![](./media/Wireshark_2.png)
3. Каково значение исходного Ethernet-адреса? Какое устройство имеет такой Ethernet-адрес?
   - `50:ff:20:33:01:ea`
   - ![](./media/Wireshark_3.png)
   - Это мой домашний маршрутизатор Keenetic. Но на его задней стороне последняя буква B, а не A. xD Либо кто-то ошибся, либо я что-то не понимаю...
   - ![](./media/Wireshark_3a.png)
4. Каков адрес назначения в этом Ethernet-кадре? Это Ethernet-адрес вашего компьютера?
   - Да, это MAC-адрес моего ноутбука:
   - ![](./media/Wireshark_1.png)
...
1. Каковы шестнадцатеричные значения исходного и конечного адресов в Ethernet-кадре,
   ...
   - ![](./media/Wireshark_4.png) 
   - Source это мой ноутбук. Destination -- все единицы, то есть широковещательная рассылка.
2. Содержится ли в ARP-сообщении IP-адрес отправителя?
   - ![](./media/Wireshark_5.png)
   - Да, содержится.
3. Где в ARP-запросе находится сама «вопросная часть» – Ethernet-адрес той машины, чей
   соответствующий IP-адрес мы запрашиваем?
   - ![](./media/Wireshark_6.png)
   - Вот в этом поле target MAC address везде нули: тут хотим получить ответ - настоящий MAC адрес узла с IP-адресом 192.168.1.1.
4. Где в сообщении с ARP-откликом хранится Ethernet-адрес, который мы запрашивали на
   основе соответствующего IP-адреса?
   - ![](./media/Wireshark_7.png)
   - В поле sender MAC address. И это MAC-адрес моего маршрутизатора!
